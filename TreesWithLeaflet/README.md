...
1) Now, outside of the command prompt/ Terminal, right-click on the orignal `index.html` file and rename it `finalScript.html`. 
2) Right-click on the original `shell.html` file and rename it `index.html`. We will be building our map in the new `index.html` now. 
